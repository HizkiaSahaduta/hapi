...
class HomeController extends Controller
{
    ...
    public function index(){
        ...
        // $curr = Carbon::now();
        // $month =  date("F", mktime(0, 0, 0, $curr->month, 1));
        // $subMonth = date("F", mktime(0, 0, 0, $curr->subMonth()->month, 1));
        // $year =  $curr->year;
        // $subYear =  $curr->subYear()->year;

        // return view('layouts.Home',['year' => $year, 'subYear' => $subYear, 'month' => strtoupper($month), 'subMonth' => strtoupper($subMonth)]);
        ...
        switch($groupid)
        {
            case 'DEVELOPMENT';
                ...
                $OrderPrcentageLastMonth = DB::connection("sqlsrv2")
                                        ->table('dashboard_order_prod')
                                        ->selectRaw("category, format(convert(decimal(10,4),(sum(total_order)/".$divider.") * 100),'N4') persentase")
                                        ->where('year', '=', $year)
                                        ->groupBy('category')
                                        ->orderBy('category')
                                        ->get();
